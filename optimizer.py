...
class DrinkPreference:
    ...
    def get_available_containers(self) -> List[ContainerOption]:
        """Get only the containers that have prices specified"""
        return [c for c in self.category.container_options 
                if c.name in self.price_per_container]
...
class PreferenceConfig:
    ...
    def _validate_config(self):
        ...
        for drink in self.config['drink_preferences']:
            ...
            # Check that all specified prices correspond to valid containers
            invalid_containers = price_names - container_names
            if invalid_containers:
                raise ValueError(
                    f"Invalid container sizes for {drink['name']}: {invalid_containers}. "
                    f"Valid containers for {drink['category']}: {container_names}"
                )
            # Ensure at least one container has a price
            if not price_names:
                raise ValueError(f"No prices specified for {drink['name']}")
...
def optimize_alcohol_purchases(
    ...
    for pref in preferences:
        cat_name = pref.category.name
        category_ratio = pref.votes / category_votes[cat_name] if pref.votes > 0 else 0
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python optimizer.py <config_path>")
        sys.exit(1)
    main(sys.argv[1])